...
/* ---------------- Bouton d'action avec mode d'emploi ("?") ---------------- */

function actionButton({ label, onclick, help, cls = 'btn', title, type = 'button', disabled = false }) {
  const attrs = { class: cls, type, onclick };
  if (title) attrs.title = title;
  if (disabled) attrs.disabled = 'disabled';
  const btn = h('button', attrs, label);
  if (!help) return btn;
  return h('span', { class: 'btn-with-help' }, [btn, helpButton(help)]);
}
...
function checkboxGroup({ options, values, onChange }) {
  ...
  options.forEach((opt, i) => {
    ...
    cb.checked = !!values[i];
    cb.addEventListener('change', () => onChange(i, cb.checked));
...
function bindGlobalActions() {
  document.getElementById('btn-import').insertAdjacentElement('afterend', helpButton(
    "Importe un fichier JSON précédemment généré avec « Exporter tout » — remplace toutes les données actuellement stockées dans ce navigateur par celles du fichier. Utile pour restaurer une sauvegarde ou transférer les données vers un autre poste."
  ));
  document.getElementById('btn-export').insertAdjacentElement('afterend', helpButton(
    "Télécharge un fichier JSON contenant toutes les affaires enregistrées dans ce navigateur — à conserver comme sauvegarde régulière ou pour transférer les données vers un autre navigateur/poste."
  ));
...
function render() {
  ...
  if (parts[0] === 'affaire' && parts[1]) {
    ...
    renderReperes($main);
  } else if (parts[0] === 'cas-pratique') {
    setActiveNav('cas-pratique');
    renderCasPratique($main, parts[1] || 'configuration');
  } else {
    setActiveNav('dashboard');
...
function renderDashboard(container) {
  ...
  toolbar.appendChild(actionButton({
    ...
    help: "Crée une nouvelle affaire vide et vous ouvre directement sur sa fiche d'identité pour commencer la saisie."
  }));
  ...
  function renderList() {
    ...
    items.forEach(a => {
      ...
      actions.appendChild(actionButton({
        cls: 'btn btn-light btn-sm', title: 'Dupliquer',
        onclick: () => { const c = Store.duplicateAffaire(a.id); navigate(`/affaire/${c.id}/identite`); },
        label: 'Dupliquer',
        help: "Crée une copie complète de cette affaire (toutes les données) avec un nouvel identifiant — pratique pour repartir d'une affaire similaire."
      }));
      actions.appendChild(actionButton({
        cls: 'btn btn-light btn-sm', title: 'Exporter en Excel',
        onclick: () => exportAffaireToXlsx(a),
        label: 'Excel',
        help: "Télécharge un classeur Excel (.xlsx) de cette affaire, avec un onglet par section (Identité, RDV, Score 40, Qualification, Fiche Mission, Points de contrôle)."
      }));
      actions.appendChild(actionButton({
        cls: 'btn btn-light btn-sm',
        onclick: () => { Store.setArchived(a.id, !a.archived); renderList(); showToast(a.archived ? 'Désarchivée' : 'Archivée'); },
        label: a.archived ? 'Désarchiver' : 'Archiver',
        help: a.archived
          ? "Fait réapparaître cette affaire dans la liste par défaut."
          : "Masque cette affaire de la liste par défaut sans la supprimer — retrouvable en cochant « Afficher les affaires archivées »."
      }));
      actions.appendChild(actionButton({
        ...
        label: 'Supprimer',
        help: "Supprime définitivement cette affaire et toutes ses données (RDV, évaluations, qualification, mission). Une confirmation est demandée — cette action est irréversible."
      }));
...
function renderAffaireEditor(container, id, tab) {
  ...
  actions.appendChild(actionButton({
    cls: 'btn btn-light', onclick: () => exportAffaireToXlsx(a), label: 'Exporter en Excel',
    help: "Télécharge un classeur Excel (.xlsx) de cette affaire, avec un onglet par section (Identité, RDV, Score 40, Qualification, Fiche Mission, Points de contrôle) — pour archiver ou partager hors de l'application."
  }));
  actions.appendChild(actionButton({
    cls: 'btn btn-light',
    ...
    help: a.archived
      ? "Fait réapparaître cette affaire dans la liste par défaut."
      : "Masque cette affaire de la liste par défaut sans la supprimer — retrouvable depuis le tableau de bord en cochant « Afficher les affaires archivées »."
  }));
...
function renderTabRdv(panel, a, id) {
  ...
  subtabs.appendChild(actionButton({
    cls: 'btn btn-primary btn-sm',
    onclick: () => { const r = Store.addRdv(id); uiState.rdv[id] = r.id; render(); },
    label: '+ Nouveau RDV',
    help: "Ajoute un nouveau rendez-vous à préparer/suivre pour cette affaire — les rendez-vous précédents restent consultables dans les onglets ci-dessus."
  }));
  ...
  panel.appendChild(grid([
    ...
    field({ label: 'Type de rendez-vous', value: r.typeRdv, onChange: updR('typeRdv'), type: 'select', options: TYPE_RDV_OPTIONS }),
  ...
  panel.appendChild(grid([
    field({ label: 'Nom', value: r.interNom, onChange: updR('interNom') }),
    ...
    field({ label: 'Prénom', value: r.interPrenom, onChange: updR('interPrenom') }),
...
function renderTabScore40(panel, a, id) {
  ...
  subtabs.appendChild(actionButton({
    ...
    help: "Crée une nouvelle évaluation Score 40, indépendante des précédentes — l'historique en bas de page se met à jour automatiquement à partir de toutes les évaluations."
  }));
  panel.appendChild(subtabs);
...
function renderTabMission(panel, a, id) {
  ...
  panel.appendChild(h('div', { class: 'field span-2' }, [
    h('label', {}, [h('span', { class: 'label-text' }, 'Pourquoi gagner ?'), helpButton('Cochez toutes les raisons qui s\'appliquent à cette affaire.')]),
    checkboxGroup({
      options: POURQUOI_GAGNER_OPTIONS, values: m.pourquoiGagner,
      onChange: (i, checked) => Store.updateAffaire(id, aff => { aff.mission.pourquoiGagner[i] = checked; })
    })
  ]));

  panel.appendChild(sectionTitle('3. Périmètre technique'));
  panel.appendChild(grid([
    field({ label: 'Type de projet', value: m.typeProjet, onChange: upd('typeProjet'), type: 'select', options: TYPE_PROJET_OPTIONS }),
    field({ label: 'Type de marché', value: m.typeMarche, onChange: upd('typeMarche'), type: 'select', options: TYPE_MARCHE_OPTIONS }),
    field({ label: 'Référentiel', value: m.referentiel, onChange: upd('referentiel'), type: 'select', options: REFERENTIEL_OPTIONS }),
    field({ label: 'Activité', value: m.activite, onChange: upd('activite'), type: 'select', options: ACTIVITE_OPTIONS }),
    field({ label: 'Planning client', value: m.planningClient, onChange: upd('planningClient') }),
  ...
  if (niveauSuggere && niveauSuggere !== m.niveauDemande) {
    panel.appendChild(h('div', { class: 'suggestion-box' }, [
      ...
      actionButton({
        ...
        label: 'Appliquer',
        help: "Reporte automatiquement dans « Niveau demandé » le niveau d'étude (N1/N2/N3) suggéré par la couleur de cette affaire."
      })
    ]));
  }
  ...
  panel.appendChild(h('div', { class: 'field span-2' }, [
    h('label', {}, [h('span', { class: 'label-text' }, 'Préparation du chiffrage'), helpButton('Cochez les tâches de préparation déjà réalisées ou à prévoir pour ce chiffrage.')]),
    checkboxGroup({
      options: PREPARATION_CHIFFRAGE_OPTIONS, values: m.preparationChiffrage,
      onChange: (i, checked) => Store.updateAffaire(id, aff => { aff.mission.preparationChiffrage[i] = checked; })
    })
  ]));
  ...
  function renderVigRows() {
    ...
    m.vigilances.forEach((v, i) => {
      ...
      const rmBtn = actionButton({
        ...
        label: '✕', title: 'Supprimer cette ligne de vigilance'
      });
  ...
  panel.appendChild(h('div', { class: 'repeat-table-actions' }, actionButton({
    ...
    help: "Ajoute une ligne vide au tableau des points de vigilance transmis au chiffreur."
  })));
...
function renderTabAtoutCom(panel, a, id) {
  ...
  exportActions.appendChild(actionButton({
    cls: 'btn btn-light btn-sm',
    onclick: async () => {
    ...
    label: 'Copier',
    help: "Copie le Bloc de Reprise ci-dessus dans le presse-papiers, prêt à coller dans une conversation Copilot ATOUT_COM."
  }));
  exportActions.appendChild(actionButton({
    cls: 'btn btn-light btn-sm',
    onclick: () => saveFile(`bloc-reprise-${(a.identite.client || 'affaire').replace(/[^a-z0-9]+/gi, '_')}.txt`, exportArea.value, 'text/plain'),
    label: 'Télécharger (.txt)',
    help: "Télécharge le Bloc de Reprise sous forme de fichier texte (.txt) — utile si le copier-coller n'est pas disponible."
  }));
  ...
  panel.appendChild(h('div', { style: 'margin-top:8px' }, actionButton({
    ...
    label: 'Analyser',
    help: "Analyse le texte collé ci-dessus et propose un aperçu des informations détectées — rien n'est modifié sur l'affaire tant que vous n'avez pas cliqué sur un bouton « Appliquer… » ci-dessous."
  })));
...
function renderImportPreview(box, a, id, parsed) {
  ...
  if (parsed.couleur) {
    actions.appendChild(actionButton({
      ...
      label: 'Appliquer la couleur',
      help: "Met à jour la couleur (VERT/ORANGE/ROSE) de cette affaire dans l'onglet Qualification avec la couleur détectée dans le texte importé."
    }));
  }
  if (parsed.score40Notes) {
    actions.appendChild(actionButton({
      ...
      label: 'Créer une évaluation Score 40',
      help: "Crée une nouvelle évaluation Score 40 à partir des notes détectées dans le texte importé, sans toucher aux évaluations existantes."
    }));
  }
  if (parsed.client && !a.identite.client) {
    actions.appendChild(actionButton({
      ...
      label: "Appliquer à l'identité",
      help: "Renseigne le client (et le montant si détecté) sur l'identité de l'affaire — proposé uniquement car ces champs sont encore vides."
    }));
  }
  actions.appendChild(actionButton({
    cls: 'btn btn-light btn-sm',
    ...
    label: 'Archiver ce texte sur l’affaire',
    help: "Conserve ce texte tel quel sur l'affaire, sans en extraire ni appliquer les données — consultable plus tard dans « Dernier bloc importé (archive) »."
  }));
