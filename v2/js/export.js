...
function buildRdvSheet(a) {
  ...
  a.rdvs.forEach((r, i) => {
    ...
    aoa.push(
      ...
      [],
      ["2 — L'INTERLOCUTEUR"],
      ['Nom', r.interNom, "Niveau d'influence perçu", r.interInfluence],
      ['Prénom', r.interPrenom],
...
function buildMissionSheet(a) {
  ...
  const aoa = [
    ...
    ['Concurrents', m.concurrents, 'Budget client', m.budgetClient],
    ['Pourquoi gagner ?', POURQUOI_GAGNER_OPTIONS.filter((o, i) => m.pourquoiGagner[i]).join(', ') || '—'],
    [],
    ['3. PÉRIMÈTRE TECHNIQUE'],
    ['Type de projet', m.typeProjet, 'Type de marché', m.typeMarche],
    ['Référentiel', m.referentiel, 'Activité', m.activite],
    ['Planning client', m.planningClient, 'Documents dispo', m.documentsDispo],
    [],
    ['4. COMMANDE AU CHIFFREUR'],
    ['Niveau demandé', m.niveauDemande, 'Heures allouées', m.heuresAllouees],
    ['Préparation du chiffrage', PREPARATION_CHIFFRAGE_OPTIONS.filter((o, i) => m.preparationChiffrage[i]).join(', ') || '—'],
    [],
