...
const STORAGE_KEY = 'agentCeng.affaires.v2';
...
const TYPE_RDV_OPTIONS = ['Découverte', 'Mise au point', 'Présentation offre', 'Négociation', 'Closing'];

const POURQUOI_GAGNER_OPTIONS = [
  'Gagner un client stratégique', 'Faire une référence', 'Développer les Travaux',
  'Développer la Maintenance', 'Assurer la rentabilité', 'Autres'
];

const TYPE_PROJET_OPTIONS = ['Industrie', 'Logistique', 'Tertiaire', 'Data center', 'Parking', 'ERP', 'RS'];
const REFERENTIEL_OPTIONS = ['NFPA', 'FMG', 'APSAD', 'NF_EN'];
const TYPE_MARCHE_OPTIONS = ['Marché Public', 'Marché Privé'];
const ACTIVITE_OPTIONS = ['Lot unique', 'Clé en main', 'MACRO-lot', 'Groupement'];

const PREPARATION_CHIFFRAGE_OPTIONS = [
  "Note d'hypothèses", 'Analyse de risques', 'Optimisations techniques', 'Variantes techniques',
  'Variantes économiques', 'Analyse compétitivité', 'Préparation offre', 'Soutenance client', 'CENG'
];
...
const QUALIFICATION_QUESTIONS = [
  ...
  { label: 'Le client dispose-t-il d\'un projet, sait-il exactement ce qu\'il veut ?', options: [
    { label: 'Oui', orange: 1, rose: 0 }, { label: 'Non', orange: 0, rose: 1 }
  ] },
...
function newRdv() {
  return {
    ...
    interNom: '', interPrenom: '', interFonction: '', interRole: '', interStatut: '',
...
function newMission() {
  return {
    ...
    indiceConfiance: '', concurrents: '', budgetClient: '',
    pourquoiGagner: POURQUOI_GAGNER_OPTIONS.map(() => false),
    typeProjet: '', activite: '', referentiel: '', typeMarche: '', documentsDispo: '', planningClient: '',
    niveauDemande: '', heuresAllouees: '',
    preparationChiffrage: PREPARATION_CHIFFRAGE_OPTIONS.map(() => false),
